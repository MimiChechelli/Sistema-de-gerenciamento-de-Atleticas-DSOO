...
from dao.partidaDao import PartidaDAO
...
class ControladorPartida():
    ...
    def pega_partida_pelos_dados(self, codigo):
        return self.__tela_partida.get(codigo)
    ...
    def incluir_partida(self):
        ...
        if p is None:
            partida = Partida(codigo, data_partida, atletica_1,
                              atletica_2, arbitro, resultado_atl_1, resultado_atl_2)
            self.__partida_dao.add(partida)
            self.__tela_partida.mostra_mensagem("Partida cadastrada\n")
        else:
            self.__tela_partida.mostra_mensagem(
                "ATENCAO: Partida já existente")
    ...
    def alterar_partida(self):
        ...
        if p.codigo == codigo:
            ...
            self.__partida_dao.add(p)  # atualizar o DAO
            self.__tela_partida.mostra_mensagem("Partida alterada")
        else:
            self.__tela_partida.mostra_mensagem("Partida não encontrada")

    def excluir_partida(self):
        codigo = self.__tela_partida.pegar_codigo_partida()
        if self.partoda_dao.remove(codigo):
